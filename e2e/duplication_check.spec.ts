...
test("Options should not be duplicated in role categories", async ({ page }) => {
	await page.getByRole("button", { name: "ゴーストニュートラル役職設定", exact: true }).click();

	const forasCategory = page.getByTestId("exr-category-521");
	await expect(forasCategory).toBeVisible();

	// Enable by setting rate to 10%
	const rateSlider = forasCategory.getByTestId("spawn-rate-control").locator('input[type="range"]');
	await rateSlider.fill("1");

	// Open accordion
	await forasCategory.getByRole("button", { name: "フォラス" }).click();

	const content = forasCategory.locator('[data-testid="exr-category-list-container"]');
