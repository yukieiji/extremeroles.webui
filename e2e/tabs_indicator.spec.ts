...
test.describe("Tabs Indicator Styling", () => {
	test("Au Tab active indicator should have correct color", async ({ page }) => {
        ...
        const colorVar = await tab1.evaluate((el) => (el as HTMLElement).style.getPropertyValue('--tab-color').trim());
        ...
        await tab2.click();
        const colorVar2 = await tab2.evaluate((el) => (el as HTMLElement).style.getPropertyValue('--tab-color').trim());
        expect(colorVar2.toLowerCase()).toMatch(/^(rgb\(255, 0, 0\)|#ff0000)$/);
	});

    test("ExR Tab active indicator should support gradients", async ({ page }) => {
        // ExR Options is default
        await page.getByRole("button", { name: "ExR Options" }).click();

        // General Tab
        const generalTab = page.getByRole("tab").first();
        const colorVar = await generalTab.evaluate((el) => (el as HTMLElement).style.getPropertyValue('--tab-color').trim());
        ...
        for (let i = 0; i < count; i++) {
            ...
            const color = await tab.evaluate((el) => (el as HTMLElement).style.getPropertyValue('--tab-color').trim());
            if (color) {
