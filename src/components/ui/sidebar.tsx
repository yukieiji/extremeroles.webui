	...
	if (collapsible === "none") {
		return (
			<div
				data-slot="sidebar"
				className={cn(
					"flex h-full w-(--sidebar-width) flex-col bg-n4-components-background text-text-primary",
					className,
				)}
				{...props}
			>
		...
		return (
			<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
				<SheetContent
					...
					data-slot="sidebar"
					data-mobile="true"
					className="w-(--sidebar-width) bg-n4-components-background p-0 text-text-primary [&>button]:hidden"
					style={
						{
			...
			<div
				...
				className={cn(
					...
					variant === "floating" || variant === "inset"
						...
						: "group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:border-r group-data-[side=left]:border-border-strong group-data-[side=right]:border-l group-data-[side=right]:border-border-strong",
					className,
				)}
				{...props}
			>
				<div
					...
					className="flex size-full flex-col bg-n4-components-background group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-lg group-data-[variant=floating]:ring-2 group-data-[variant=floating]:ring-border-strong"
				>
					{children}
				</div>
			</div>
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
	return (
		<main
			...
			className={cn(
				"relative flex w-full flex-1 flex-col bg-app-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
				className,
			)}
			{...props}
		/>
...
function SidebarSeparator({
	...
	return (
		<Separator
			...
			className={cn("mx-2 w-auto bg-border-weak", className)}
			{...props}
		/>
	);
}
	...
	"peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-component-hover focus-visible:ring-2 active:bg-component-hover disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-component-hover data-active:bg-component-hover data-active:font-medium [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
	{
		variants: {
			variant: {
				...
				outline:
					"bg-n4-components-background shadow-lg ring-2 ring-border-strong hover:bg-component-hover",
			},
			size: {
				default: "h-8 text-sm",
				sm: "h-7 text-xs",
...
function SidebarMenuBadge({
	...
	return (
		<div
			...
			className={cn(
				"pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium text-text-primary tabular-nums select-none group-data-[collapsible=icon]:hidden peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5 peer-data-[size=sm]/menu-button:top-1",
				className,
			)}
			{...props}
		/>
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
	return (
		<ul
			...
			className={cn(
				"mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-border-weak px-2.5 py-0.5 group-data-[collapsible=icon]:hidden",
				className,
			)}
			{...props}
		/>
